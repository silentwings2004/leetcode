package LC2700_3000;
import java.util.*;
public class LC2907_MaximumProfitableTripletsWithIncreasingPricesI {
    /**
     ...
     * If we pick items with indices i, j and k satisfying the above condition, the profit would be
    ...
    public int maxProfit(int[] prices, int[] profits) {
        ...
        for (int j = 0; j < n; j++) {
            for (int i = j - 1; i >= 0; i--) {
                if (prices[i] < prices[j]) l[j] = Math.max(l[j], profits[i]);
            }
        }
        for (int j = n - 1; j >= 0; j--) {
            for (int k = j + 1; k < n; k++) {
                if (prices[k] > prices[j]) r[j] = Math.max(r[j], profits[k]);
            }
        }
        int res = -1;
        for (int i = 0; i < n; i++) {
            if (l[i] != 0 && r[i] != 0) res = Math.max(res, l[i] + profits[i] + r[i]);
        }
        return res;
    }
    ...
    List<Integer> nums;
    int m;
    public int maxProfit2(int[] prices, int[] profits) {
        tr = new int[N];
        int n = prices.length;
