...
public class LC1600_ThroneInheritance {
     ...
     * Calling Successor(Jack, curOrder) will return Bob, so we append to curOrder to get
     * ["king", "Alice", "Jack", "Bob"].
    ...
    public List<String> getInheritanceOrder() {
        res = new ArrayList<>();
        if (!set.contains(king)) res.add(king);
        dfs(king);
        return res;
    }
    ...
    private void dfs(String s) {
        ...
        for (String son : map.get(s)) {
            if (!set.contains(son)) res.add(son);
            dfs(son);
        }
    }
}