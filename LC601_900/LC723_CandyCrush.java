...
public class LC723_CandyCrush {
     ...
     * steps.
     * If there does not exist more candies that can be crushed (i.e., the board is stable), then return the current
     * board.
            ...
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
                    ...
                    if (j < n - 2 && Math.abs(board[i][j + 1]) == val && Math.abs(board[i][j + 2]) == val) {
                        found = true;
                        int idx = j;
                        while (idx < n && Math.abs(board[i][idx]) == val) board[i][idx++] = -val;
                    }

                    if (i < m - 2 && Math.abs(board[i + 1][j]) == val && Math.abs(board[i + 2][j]) == val) {
                        found = true;
                        int idx = i;
                        while (idx < m && Math.abs(board[idx][j]) == val) board[idx++][j] = -val;
                    }
                }
            }
            if (found) {
                for (int j = 0; j < n; j++) {
                    ...
                    for (int k = idx; k >= 0; k--) board[k][j] = 0;
                }
            }
        }
        return board;
